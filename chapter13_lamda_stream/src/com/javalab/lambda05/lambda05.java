...
public class lambda05 {
	...
	public static void main(String[] args) {
		 ...
		 * 선언하면서 바로 객체화 됨.
		 */
		Thread thread = new Thread(new Runnable() {	// 익명 구현 객체 생성
			@Override
			public void run() {
				for (int i = 1; i <= 5; i++) {
		...
		//	       자바에서 Runnalbe이라는 함수형 인터페이스를 미리 만들어 둚.
		Runnable runnable1 = new Runnable() {
			@Override
			public void run() {
				for (int i = 1; i <= 10; i++) {
					System.out.println("runnalbe1 : " + i);
				}
			}
		};
		// 위에서 Runnable을 구현한 runnable1, runnable2
		// Thread 생성자에 Runnalbe 인터페이스 타입 변수
		...
		Thread thread3 = new Thread(() -> {	// Runnalbe을 구현한 익명 구현 객체 생성
			for (int i = 1; i <= 5; i++) {
